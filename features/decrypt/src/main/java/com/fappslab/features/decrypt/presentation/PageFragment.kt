...
import com.fappslab.features.decrypt.presentation.extension.inputError
import com.fappslab.features.decrypt.presentation.extension.numberedSeed
...
import com.fappslab.features.decrypt.presentation.extension.showUnlockSeedErrorDialog
...
internal class PageFragment : Fragment(R.layout.decrypt_fragment_page), KoinLazy {
    ...
    private fun setupObservables() = binding.run {
        onViewState(viewModel) { state ->
            ...
            inputLayoutUnreadableSeed.inputError(state.inputErrorPair)
            ...
            state.showUnlockSeedErrorDialogState()
        }

        onViewAction(viewModel) { action ->
            when (action) {
                DecryptViewAction.Validation -> showValidationDialogAction()
                DecryptViewAction.ClearInput -> clearInputAction()
                ...
                is DecryptViewAction.UnlockedSeed -> unlockedSeedAction(action.readableSeed)
            }
        }
    }
    ...
    private fun DecryptViewState.showUnlockSeedErrorDialogState() {
        showUnlockSeedErrorDialog(
            dialogErrorPair,
            shouldShowUnlockSeedErrorModal,
            viewModel::onUnlockSeedErrorVisibilityDialog
        )
    }
    ...
    private fun clearInputAction() = binding.run {
        inputUnreadableSeed.text?.clear()
        seedContainer.isVisible = false
    }

    private fun unlockedSeedAction(readableSeed: String) {
        binding.run {
            textReadableSeed.numberedSeed(readableSeed)
